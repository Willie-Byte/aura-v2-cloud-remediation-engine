...
Aura V2 is currently **demo ready with safety boundaries**.

The latest validated status is:
...
Start here for the final demo/handoff materials:

```text
backend/docs/aura-v2-final-demo-handoff.md
...
The validated live Tetragon pipeline is:
...
```text
Production remediation execution is disabled.
Terraform apply is disabled.
Destructive Kubernetes actions are not part of the demo.
Direct RAG-to-live-telemetry automation is disabled.
The system stops at human approval for the validated live flow.
```
...
The current demo should use these documents as the main handoff path:

| Document | Purpose |
|---|---|
| `backend/docs/aura-v2-final-demo-handoff.md` | Short final handoff guide for what to open, what to demo, and what not to enable. |
| `backend/docs/aura-v2-demo-readiness-summary.md` | Summary of what is demo-ready and what remains disabled. |
| `backend/docs/tetragon-live-pipeline-final-status.md` | Final status for the validated live Tetragon pipeline. |
| `AURA_V2_DEMO_CHECKLIST.md` | Full step-by-step checklist and troubleshooting reference. |


## Current Build Status

Aura V2 is still a prototype, but the current `main` branch is demo-ready with explicit safety boundaries.

Completed demo-ready capabilities include:
...
- local Vector RAG with Qdrant and OpenAI embeddings
- Tetragon local safety test suite
- AKS dry-run recovery documentation
- controlled live Tetragon validation documentation
- downstream normalizer validation documentation
- final live Tetragon pipeline status documentation
- final demo readiness and handoff documentation

Still intentionally disabled or not production-ready:
...
- `raw-telemetry` receives raw telemetry events before normalization.
- `threat-ingest` receives normalized threat events for the orchestrator.
- `remediation-commands` receives remediation commands generated by the orchestrator.
- `approval-queue` receives remediation commands that require human approval.
- `approval-decisions` receives approve/reject decisions.
- `remediation-dlq` receives invalid or rejected remediation commands.
- `audit-log` receives audit events for major system actions.
- `execution-results` receives execution, rejection, duplicate-skip, and approval-gating result records.
...
Aura V2 includes a local retrieval-augmented generation layer on the current `main` branch.
...
The vector RAG system is local-first right now.
...
```bash
npm run rag:search -- "What should stay separate from the local Vector RAG system?"
```
...
```json
{
 "query": "What should stay separate from the local Vector RAG system?",
 "limit": 5,
...
```text
What should stay separate from the local Vector RAG system?
```

Expected answer:

```text
The local Vector RAG system should stay separate from AKS deployments, uncontrolled Kafka streaming workers, live Tetragon telemetry automation, Rust eBPF enforcement code, and production remediation execution.
```

## Demoing the Local Vector RAG Branch

Use this checklist to demo the local Vector RAG feature safely.

### 1. Confirm you are on a clean branch
...
Aura V2 is ready for a safe demo using the final handoff documents.
...
- execution results are published for success, rejection, duplicate skips, and approval-gated states
- local RAG can answer questions from project architecture documents and selected source-code context
- the frontend can monitor the stream lifecycle and test the local RAG system
- a real AKS pod exec event can be captured by Tetragon and published to Kafka raw telemetry
- the downstream normalizer can convert the event into a threat
- the orchestrator and worker can process the event safely
- the final live flow stops at human approval instead of production remediation
